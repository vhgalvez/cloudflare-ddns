...
🔍 ¿Qué hace?
Carga las variables desde .env (CF_API_TOKEN, ZONE_NAME, RECORD_NAMES).

Detecta la IP pública consultando Cloudflare (https://1.1.1.1/cdn-cgi/trace).

Consulta la zona DNS en Cloudflare (usando el nombre y el token).

Para cada registro A, compara si la IP actual coincide con la de Cloudflare.

Si ha cambiado, lanza un PUT a la API de Cloudflare para actualizar el registro.

Registra todo en /var/log/cloudflare-ddns.log.
